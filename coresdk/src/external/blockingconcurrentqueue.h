...
// Uses Jeff Preshing's concurrency_driver implementation (under the terms of its
...
namespace details
{
	...
	// portable + lightweight concurrency_driver implementations, originally from
...
#else
#error Unsupported platform! (No concurrency_driver wrapper available)
#endif
		...
		private:
			...
			bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1)
			{
				...
				// as threads start hitting the kernel concurrency_driver.
				int spin = 10000;
				while (--spin >= 0)
				{
				...
				if (m_sema.timed_wait((std::uint64_t)timeout_usecs))
					return true;
				// At this point, we've timed out waiting for the concurrency_driver, but the
				// count is still decremented indicating we may still be waiting on
				// it. So we have to re-adjust the count, but only if the concurrency_driver
				// wasn't signaled enough times for us too since then. If it was, we
				// need to release the concurrency_driver too.
				while (true)
				{
